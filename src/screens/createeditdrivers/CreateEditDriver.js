  ...
  const pressSubmit = async () => {
    if (name === '') {
      ...
      if (route.params.type === 'create') {
        response = await ApiService.postApiCall(
          requestData,
          token,
          API.APP_DRIVERS_REQUEST,
        );
      } else {
        let apiURL = API.APP_DRIVERS_REQUEST + route.params?.data?.id;
        response = await ApiService.upateApiWithAuthCall(
          requestData,
          token,
          apiURL,
        );
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={{flex: 1}}>
        <View style={{flex: 1, width: WIDTH, marginTop: HEIGHT / 6}}>
          <Text style={[styles.textHeader, {color: colors.green700}]}>
            {'DELIVERY ZONE'}
          </Text>
